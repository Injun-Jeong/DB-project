<%@page import="javax.security.auth.callback.ConfirmationCallback"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="accountPackage.Account"%>
<%@ page import="util.DatabaseConnection"%>
<%@ page import="java.sql.*"%>
<%@ page import="java.lang.*" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title></title>
</head>
<body>
<%
	request.setCharacterEncoding("UTF-8");
	String accountnum = (String) session.getAttribute("user_Num");
	String Pw = request.getParameter("user_Pw");
	String Phonenumber = request.getParameter("user_Phone");
	String Email = request.getParameter("user_Email");
	String Job = request.getParameter("user_Job");
	String Address = request.getParameter("user_Address");
	String Zipcode = request.getParameter("user_Zipcode");
	
	if (Pw.equals("")) {
		out.println("<script>");
		out.println("alert('비밀번호가 누락되었습니다.');");
		out.println("history.back();");
		out.println("</script>");
	}
	else {
		if (Account.change_Info(accountnum, Pw, Phonenumber, Email, Job, Address, Zipcode)) {
			out.println("<script>");
			out.println("alert('수정 완료!');");
			out.println("location.href='../view/main.jsp';");
			out.println("</script>");
		} else {
			out.println("<script>");
			out.println("alert('수정 실패.');");
			out.println("history.back();");
			out.println("</script>");
		} 
	}

%>
</body>
</html>